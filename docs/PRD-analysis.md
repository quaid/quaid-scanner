...
| **SEC-05** | **Token Permission Analysis** | Parse GitHub Actions workflows to ensure GITHUB_TOKEN permissions are set to read-all or scoped strictly (Principle of Least Privilege). | Flag workflows with default read/write permissions. |
...
| **COM-04** | **DEI Artifact Check** | Validate CODE_OF_CONDUCT.md contains enforcement contact info (not just template text). | Boolean check for email/URL in CoC file. |
...

{
  ...
  "pillars": {
    "security": {
      "score": 9.0,
      "details": { "branch_protection": true, "pinned_dependencies": true }
    },
    "governance": {
      "score": 7.5,
      "model": "Meritocracy",
